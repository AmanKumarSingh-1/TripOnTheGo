    ...
    if(isset($_GET['fetch_rooms'])) {
        ...
        if($chk_avail['checkin'] != "" && $chk_avail['checkout'] != "") {
            ...
            if($checkin_date == $checkout_date) {
                echo "<h3 class='text-center text-danger'>Check-in and Check-out dates cannot be the same.</h3>";
                exit;
            }
            else if($checkout_date <= $checkin_date) {
                echo "<h3 class='text-center text-danger'>Check-out date cannot be earlier than or equal to Check-in date.</h3>";
                exit;
            }
            else if($checkin_date < $today_date) {
                echo "<h3 class='text-center text-danger'>Check-in date cannot be earlier than today.</h3>";
                exit;
            }
        ...
        // facility list decode
        $facility_list = json_decode($_GET['facility_list'], true);
        

        // count total rooms and output variable to store room cards
        $count_rooms = 0;
        $output = "";
